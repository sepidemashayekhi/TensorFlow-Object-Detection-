    ...
    def creatbondingBox(self,image,threshold=0.5):
        ...
        if len(bboxInd) !=0:
            for i in bboxInd:
                ...
                #
                # cv2.line(image,(xmin,ymin),(xmin+lineWidth,ymax),color=classColor,thickness=5)
                # cv2.line(image, (xmin, ymin), (xmin , ymin+lineWidth), color=classColor, thickness=5)
                #
                # cv2.line(image, (xmin, ymin), (xmax - lineWidth, ymax), color=classColor, thickness=5)
                # cv2.line(image, (xmin, ymin), (xmax, ymin+ lineWidth), color=classColor, thickness=5)
